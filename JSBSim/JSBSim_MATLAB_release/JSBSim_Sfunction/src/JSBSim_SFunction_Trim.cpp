/* JSBSim_SFunction_Trim revision 1.1 
 * 07/28/10 Brian Mills
 * %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 * Revisions (most recent at top)
 * %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 * %%% Added debug output statements for Sfunction
 * %%% Added state derivatives output port 
 ...
 * 
 * Continuous States Version For Trim
 * Continuous time states version for trim utility
 * States are calculated in Simulink from state derivatives calculated in JSBSim
 * The state vector has been simplified to 12 states instead of 19.
 ...
 * Model has 5 output ports: state vector, control output vector, propulsion output vector, calculated output vector, aerodynamic output vector and state derivatives output vector.
 ...
 * State Derivatives [u-fps v-fps w-fps p-rad-sec q-rad-sec r-rad-sec h-sl-ft long-deg lat-deg phi-rad theta-rad psi-rad]
 *
 * MDL_Updates method is not used for the Trim version of the SFunction- instead the Mdl_Derivatives method is used
 * to calculate the states.
  * *************************************************************************************************************************
 * JSBSimInterface written by Agostino De Marco for use in the JSBSimMexFunction project. Some additional functions added and changes 
...
#define S_FUNCTION_NAME  JSBSim_SFunction_Trim
...
#define delta_t				mxGetPr(ssGetSFcnParam(S, 3))[0] // dt
//#define verbosity			mxGetPr(ssGetSFcnParam(S, 4))[0]
#define verbosity			ssGetSFcnParam(S, 4)
...
//#define integrator 			mxGetPr(ssGetSFcnParam(S, 6))[0] // JSBSim integrator override
// Necessary to create mxArray of initial conditions 
#define NUMBER_OF_STRUCTS (sizeof(ic)/sizeof(struct init_cond))
#define NUMBER_OF_FIELDS (sizeof(field_names)/sizeof(*field_names))

int loop_fv;
//#define NUMBER_OF_STRUCTS1 (sizeof(v)/sizeof(struct verbosity_level))
//#define NUMBER_OF_FIELDS1 (sizeof(field_names)/sizeof(*field_names))//???


JSBSim::FGFDMExec FDMExec;// Create JSBSim FDMExec object
...
static void mdlInitializeSizes(SimStruct *S)
{
  /* See sfuntmpl_doc.c for more details on the macros below */
    ...
    ssSetOutputPortWidth(S, 0, 12);//States output [u v w p q r q1 q2 q3 q4 long-deg lat-deg z-ft phi theta psi h-ft alpha beta]
	...
	ssSetOutputPortWidth(S, 3, 11);//Calculated outputs [Nz alphadot-radsec betadot-radsec mach vc-kts ve-kts hdot-fps]
    
	ssSetOutputPortWidth(S, 4, 6);//JSBSim Calculated Aerodynamic forces and moments output [X Y Z L M C]
	ssSetOutputPortWidth(S, 5, 12);//JSBSim calculated state derivatives
    ...
    ssSetDWorkWidth(     S, 1, 12);//Work vector for states * may need to add actuator states!
    ssSetDWorkDataType(  S, 1, SS_DOUBLE);

	ssSetDWorkWidth(     S, 2, 12);	//Work vector derivatives
	...
	ssSetDWorkWidth(     S, 6, ssGetOutputPortWidth(S,4));//Work vector for aerodynamic outputs
...
#define MDL_INITIALIZE_CONDITIONS   /* Change to #undef to remove function */
 ...
 /***************************************************************************************/
		...
		// mxDestroyArray(*prhs);
	    mexPrintf("\n");
	    mexPrintf("\tJSBSim S-Function Trim is initializing...\n");
		mexPrintf("\n");
		...
		{
			mexPrintf("\n\tAircraft file could not be loaded.\n");
			mexPrintf("\n");
		}
		...
		*/
		...
		{"rate_rot", 0},{"rate_trans", 0},{"pos_rot", 0}, {"pos_trans", 0}};
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	double *w2 = (double *) ssGetDWork(S,2);// get the derivative work vector
    int i;
	...
	for (i = 0; i < ssGetDWorkWidth(S,6); i++)
	 {
		y5[i] = w6[i]; // outputs are the aero outputs 
	 }
	
	for (i = 0; i < ssGetNumContStates(S); i++)
	 {
		y6[i] = w2[i]; // outputs are the state derivative outputs 
	 }
...
#define MDL_DERIVATIVES  /* Change to #undef to remove function */
  ...
  static void mdlDerivatives(SimStruct *S)
  {
	...
	JII->UpdateDerivatives(inputs, derivatives, states, controls, propulsion, outputs, aero); // call to JSBSimInterface to get updated derivatives from JSBSim
	
	 //Save the newly calculated derivatives
	...
	JII->UpdateStates(inputs, derivatives, states, controls, propulsion, outputs, aero); // call to JSBSimInterface to get updated derivatives from JSBSim
...
static void mdlTerminate(SimStruct *S)
{
	...
	JII->~JSBSimInterface();
	mexPrintf("\n");
	mexPrintf(">>>>>>>>JSBSIM_TRIM SIMULATION COMPLETED<<<<<<<<<<\n");
	mexPrintf("\n");
